        ...
        public void onMethodCall(MethodCall call, Result result) {
            ...
            methodCallDispatching: switch (call.method) {
                ...
                case "connectPlus": {
                    ...
                    AsyncTask.execute(() -> {
                        try {
                            connection.connectPlus(activeContext, address);
                            activity.runOnUiThread(() -> result.success(id));
                        } catch (Exception ex) {
                            activity.runOnUiThread(
                                    () -> result.error("connect_error", ex.getMessage(), exceptionToString(ex)));
                ...
                case "writePlus": {
                    ...
                    if (call.hasArgument("bytes")) {
                        ...
                        result.error("invalid_argument", "there must be 'bytes' argument", null);
                    }
                    break;
                }
                ...
                case "setNotify": {
                    ...
                    if (call.hasArgument("boolean")) {
                        Boolean isEnabled = call.argument("boolean");
                        AsyncTask.execute(() -> {
                            try {
                                connection.setNotify(isEnabled);
                                activity.runOnUiThread(() -> result.success(null));
                            } catch (Exception ex) {
                                activity.runOnUiThread(
                                        () -> result.error("write_error", ex.getMessage(), exceptionToString(ex)));
                        ...
                        result.error("invalid_argument", "there must be 'boolean' argument", null);
                    }
