...
class BluetoothConnection {
  ...
  static Future<BluetoothConnection> toAddressPlus(String? address) async {
    // Sorry for pseudo-factory, but `factory` keyword disallows `Future`.
    return BluetoothConnection._consumeConnectionID(await FlutterBluetoothSerial
        ._methodChannel
        .invokeMethod('connectPlus', {"address": address}));
  }
...
class _BluetoothStreamSink<Uint8List> extends StreamSink<Uint8List> {
  ...
  void add(Uint8List data) {
    ...
    _chainedFutures = _chainedFutures.then((_) async {
      ...
      await FlutterBluetoothSerial._methodChannel
          .invokeMethod('writePlus', {'id': _id, 'bytes': data});
    }).catchError((e) {
      this.exception = e;
      close();
    });
  ...
  void setNotify(bool isEnabled) {
    ...
    _chainedFutures = _chainedFutures.then((_) async {
      ...
      await FlutterBluetoothSerial._methodChannel
          .invokeMethod('setNotify', {'id': _id, 'boolean': isEnabled});
    }).catchError((e) {
      this.exception = e;
      close();
    });
